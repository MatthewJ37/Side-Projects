...
void shuffle(vector<vector<string>>& deck)
{
...
void deal(vector<vector<string>>& deck, vector<vector<string>>& hand)
{
    hand.push_back(deck.back());
    deck.pop_back();
}

int blackjack_score(vector<vector<string>>& hand)
{
    int score = 0;
    int aceCount = 0;
    for(int i = 0; i < hand.size(); i++)
    {
        string value = hand[i][0];
        if(value == "A"){
            score += 11;
            aceCount++;
        }
...
void blackjack(vector<vector<string>>& deck)
{
    ...
    cout << "Player: " << player[0][0] << player[0][1] << " " << player[1][0] << player[1][1] << endl;
    cout << "Dealer: " << dealer[0][0] << dealer[0][1] << " ?" << endl;
